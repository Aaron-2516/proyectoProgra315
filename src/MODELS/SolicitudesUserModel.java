...
public class SolicitudesUserModel {
   ...
   public boolean crearSolicitud(String descripcion, String creadaPorId, String categoriaId) {
    ...
    // Usamos un mismo Connection y reintentamos si hay colisión del PK.
    try (Connection cn = DatabaseConnection.getConnection()) {
        if (cn == null) return false;

        // Hasta 5 intentos por si hay inserciones concurrentes
...
// Obtiene el máximo numérico de los IDs tipo SOL### y devuelve el siguiente.
// Ej: si el mayor es SOL015 -> retorna SOL016. Soporta más de 3 dígitos (SOL1000, etc.).
 
private String generarSiguienteId(Connection cn) {
    final String SQL_MAX =
        // Toma la parte numérica posterior a 'SOL' y calcula el MAX como entero
    ...
    // Formato con al menos 3 dígitos (SOL001...SOL999); si pasa, crece: SOL1000, etc.
    if (siguiente < 1000) {
    ...
    // Trae datos mínimos de una solicitud para editar (valida que pertenezca al usuario)
...
// Actualiza descripción y categoría (solo si la creó el usuario)
...
// Elimina solicitud (solo si la creó el usuario)
